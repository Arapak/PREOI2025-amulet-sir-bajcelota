...
constexpr std::string_view MESSAGE_SENT_BUT_CHILD_EXITED =
    "MESSAGE_SENT_BUT_CHILD_EXITED";
constexpr std::string_view WAITING_FOR_MESSAGE_BUT_CHILD_EXITED =
    "WAITING_FOR_MESSAGE_BUT_CHILD_EXITED";
constexpr std::string_view BOTH_CHILDREN_EXITED_BUT_NO_ANSWER =
    "BOTH_CHILDREN_EXITED_BUT_NO_ANSWER";
...
const std::map<std::string_view, std::string_view> message = {
    tr{COMMUNICATION_ERROR, "Robisz coś bardzo bardzo dziwnego?"},
    {MESSAGE_LIMIT, "Limit przesyłanych wiadomości został przekroczony"},
    {BOTH_SENT, "Oba programy wysłały na raz wiadomość (musisz 'sować' send i "
                "receive w obu programach)"},
    {BOTH_WAIT, "Oba programy oczekują na raz wiadomości (musisz 'sparować' "
                "send i receive w obu programach)"},
    {MESSAGE_SENT_BUT_CHILD_EXITED,
     "Wiadomość wysłana, ale drugi proces się już zakończył"},
    {WAITING_FOR_MESSAGE_BUT_CHILD_EXITED,
     "Oczekiwanie wiadomości, ale drugi proces się już zakończył"},
    {BOTH_CHILDREN_EXITED_BUT_NO_ANSWER,
     "Oba programy się zakończył, ale żaden nie wywołał funkcji answer"},
    {INVALID_MESSAGE_TYPE, "Robisz coś bardzo bardzo dziwnego?"},
    {CHILD_INVALID_EXIT,
     "Program (dziecka) zakończył się w nieoczekiwany sposób - wywołujesz na "
     "przykład komendę exit? (powyżej wskazane "
     "z jakim kodem został zakończony program)"},
    {CHILD_SIGNALED,
     "Program (dziecka) zakończył się sygnałem - runtime error. Powyżej "
     "wskazane które dziecko oraz kod błędu "
     "(analogicznie do błędu na sio 'Runtime error - signal X')"},
    {BOTH_CHILDREN_ANSWERED_WITH_DIFFERENT_VALUES,
     "Oba programy wywołały answer, ale z różnymi odpowiedziami"}};
} // namespace ErrorType

class CommunicationError : public std::exception {};

template <typename DATA_TYPE, typename ANSWER_TYPE> class Communication {
public:
...
private:
  ...
  struct is_vector_of_supported_types<std::vector<T>>
      : std::conditional_t<is_supported_type<T>::value, std::true_type,
                           is_vector_of_supported_types<T>> {};

  template <typename T>
  std::enable_if_t<is_supported_type<T>::value, void>
  send(childId id, const T &x, bool toMaster) {
    send(&x, sizeof(x), id, toMaster);
  }

  template <typename T>
  std::enable_if_t<is_vector_of_supported_types<T>::value, void>
  send(childId id, const T &x, bool toMaster) {
  ...
  template <typename T>
  std::enable_if_t<is_supported_type<T>::value, T> receive(childId id,
                                                           bool fromMaster) {
    T result;
  ...
  template <typename T>
  std::enable_if_t<is_vector_of_supported_types<T>::value, T>
  receive(childId id, bool fromMaster) {
    auto s = receive<size_t>(id, fromMaster);
    T result(s);
    for (size_t i = 0; i < s; i++)
      result[i] = receive<typename T::value_type>(id, fromMaster);
    return result;
  }
...
private:
  ...
  void master_exitGracefully(const std::string_view &exitType,
                             std::string_view errorType, auto... messages) {
...
template <typename D, typename A>
void Communication<D, A>::send(const void *arr, size_t size, childId child_id,
                               bool toMaster) {
  size_t sent = 0;
  if (toMaster) {
    if (!writeAll(pipeToMaster[child_id][1], (char *)arr + sent, size))
      throw CommunicationError();
  } else if (!writeAll(pipeFromMaster[child_id][1], (char *)arr + sent, size)) {
    if (!master_checkChildStatus(child_id))
      throw CommunicationError();
  }
}

template <typename D, typename A>
void Communication<D, A>::receive(void *arr, size_t size, childId child_id,
                                  bool fromMaster) {
  size_t received = 0;
  if (fromMaster) {
    if (!readAll(pipeFromMaster[child_id][0], (char *)arr + received, size))
      throw CommunicationError();
  } else if (!readAll(pipeToMaster[child_id][0], (char *)arr + received,
                      size)) {
    if (!master_checkChildStatus(child_id))
      throw CommunicationError();
  }
...
bool Communication<D, A>::master_checkChildStatus(childId id) {
  ...
  if (ret > 0) {
    if (WIFEXITED(status))
      master_exitGracefully(ExitType::ERROR, ErrorType::CHILD_INVALID_EXIT,
                            childName(id), status / 256);
    else if (WIFSIGNALED(status))
      master_exitGracefully(ExitType::ERROR, ErrorType::CHILD_SIGNALED,
                            childName(id), WTERMSIG(status));
    return true;
  }
  return false;
}
...
std::tuple<ANSWER_TYPE, int, size_t>
Communication<DATA_TYPE, ANSWER_TYPE>::masterMain() {
  assert(childPids[0] != 0 && childPids[1] != 0 &&
         "before masterMain run forkAndRunChildren");
  ...
  for (; messages <= MESSAGE_LIMIT;) {
    ...
    std::cerr << messages << ". ";
    std::cerr << childName(CHILD_A) << "(" << std::setw(12)
              << childStatusName(status[CHILD_A]) << ") ";
    std::cerr << childName(CHILD_B) << "(" << std::setw(12)
              << childStatusName(status[CHILD_B]) << ") ";
    std::cerr << "-- " << childName(running_child) << " sent message type "
              << messageTypeName(type) << std::endl;
#endif
    ...
    case messageType::DATA:
      ...
      if (status[otherChild(running_child)] == childStatus::EXIT_WO_ANSWER)
        master_exitGracefully(ExitType::ERROR,
                              ErrorType::MESSAGE_SENT_BUT_CHILD_EXITED);
    ...
    case messageType::WAITING:
      ...
      if (status[otherChild(running_child)] == childStatus::EXIT_WO_ANSWER)
        master_exitGracefully(ExitType::ERROR,
                              ErrorType::WAITING_FOR_MESSAGE_BUT_CHILD_EXITED);
    ...
    if (std::any_of(status.begin(), status.end(),
                    [](const childStatus &x) {
                      return x == childStatus::EXIT_WITH_ANSWER;
                    }) ||
        std::all_of(status.begin(), status.end(), [](const childStatus &x) {
          return x == childStatus::EXIT_WO_ANSWER;
        })) {
      break;
    }
  ...
  if (!optional_answer.has_value()) {
    master_exitGracefully(ExitType::ERROR,
                          ErrorType::BOTH_CHILDREN_EXITED_BUT_NO_ANSWER);
  }
  ...
  if (status[otherChild(running_child)] == childStatus::RUNNING) {
    ...
    if (type == messageType::ANSWER) {
      auto other_answer =
          receive<ANSWER_TYPE>(otherChild(running_child), false);
      if (!(answer == other_answer))
        master_exitGracefully(
            ExitType::ERROR,
            ErrorType::BOTH_CHILDREN_ANSWERED_WITH_DIFFERENT_VALUES);
    }
  }
...
void Communication<DATA_TYPE, ANSWER_TYPE>::waitForAcknowledgment() {
  ...
  // allow WAITING and DATA status, because other process can expect data or
  // send data
  if (status == messageType::DATA)
    receive<DATA_TYPE>(currentId, true);
  if (status == messageType::WAITING || status == messageType::DATA)
    status = receive<messageType>(currentId, true);
